...
Function RegWrite(key, val, section=invalid) As Boolean
    if section = invalid then section = "Default"
    sec = CreateObject("roRegistrySection", section)
    sec.Write(key, val)
    return sec.Flush()
...
Function RegDelete(key, section=invalid) As Boolean
    if section = invalid then section = "Default"
    sec = CreateObject("roRegistrySection", section)
    sec.Delete(key)
    return sec.Flush()
...
Sub LogAnalytics(pageTitle, pageURL, userValue)
	GATrackPageView(pageTitle, "/" + pageURL, userValue)
End Sub

Sub LogAnalyticsEvent(pageTitle, pageURL, userValue, eventCategory, eventAction, eventLabel, eventValue)
	GATrackEvent(pageTitle, "/" + pageURL, userValue, eventCategory, eventAction, eventLabel, eventValue)
End Sub
